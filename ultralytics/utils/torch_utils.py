...
def get_cpu_info():
    ...
    if "cpu_info" not in PERSISTENT_CACHE:
        try:
            ...
            string = info.get(k[0] if k[0] in info else k[1]
                              if k[1] in info else k[2], "unknown")
            PERSISTENT_CACHE["cpu_info"] = string.replace(
...
def model_info_for_loggers(trainer):
    ...
    if trainer.args.profile:  # profile ONNX and TensorRT times
        ...
        results = ProfileModels(
            [trainer.last], device=trainer.device).profile()[0]
    ...
    results["model/speed_PyTorch(ms)"] = round(
        trainer.validator.speed["inference"], 3)
    return results
...
def get_flops_with_torch_profiler(model, imgsz=640):
    ...
    try:
        # Use stride size for input tensor
        stride = (max(int(model.stride.max()), 32) if hasattr(
            model, "stride") else 32) * 2  # max stride
        # input image in BCHW format
        im = torch.empty((1, p.shape[1], stride, stride), device=p.device)
        ...
        for m in ops if isinstance(ops, list) else [ops]:
            ...
            try:
                for _ in range(n):
                    ...
                    t[1] = time_sync()
                    try:
                        (sum(yi.sum() for yi in y) if isinstance(
