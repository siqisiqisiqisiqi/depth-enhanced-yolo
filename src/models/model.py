...
from src.utils.parse_model import parse_model
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def __init__(self, m, ch=3, nc=None, verbose=True):
        ...
        self.overrides = m.overrides
        self.task = m.task
        super().__init__(cfg=m.cfg, ch=ch, nc=nc, verbose=verbose)
...
class RTDETRPoseModel(RTDETRDetectionModel):
    ...
    def init_criterion(self):
        """Initialize the loss criterion for the RTDETRDetectionModel."""
        from ultralytics.models.utils.loss import RTDETRPoseLoss

        return RTDETRPoseLoss(nc=self.nc, use_vfl=True, with_kpts=True)
    ...
    def loss(self, batch, preds=None):
        ...
        targets = {
            "cls": batch["cls"].to(img.device, dtype=torch.long).view(-1),
            "bboxes": batch["bboxes"].to(device=img.device),
            "keypoints": batch["keypoints"].to(device=img.device),
            "batch_idx": batch_idx.to(img.device, dtype=torch.long).view(-1),
            "gt_groups": gt_groups,
        }

        preds = self.predict(img, batch=targets) if preds is None else preds
        preds_detect, dec_kpts = preds
        dec_bboxes, dec_scores, enc_bboxes, enc_scores, enc_kpts, dn_meta, _ = preds_detect if self.training else preds_detect[1]
        if dn_meta is None:
            dn_bboxes, dn_scores, dn_kpts = None, None, None
        else:
            dn_bboxes, dec_bboxes = torch.split(
                dec_bboxes, dn_meta["dn_num_split"], dim=2)
            dn_scores, dec_scores = torch.split(
        ...
        loss = self.criterion(
            (dec_bboxes, dec_scores, dec_kpts), targets, dn_bboxes=dn_bboxes, 
            dn_scores=dn_scores, dn_kpts=dn_kpts, dn_meta=dn_meta
        )
        # NOTE: There are like 12 losses in RTDETR, backward with all losses but only show the main three losses.
        return sum(loss.values()), torch.as_tensor(
            [loss[k].detach() for k in ["loss_giou", "loss_class", 
                                        "loss_bbox", "loss_kpts", "loss_oks"]], device=img.device
        )
